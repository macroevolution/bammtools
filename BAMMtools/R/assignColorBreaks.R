assignColorBreaks <- function(rates, NCOLORS = 64, spex = "s", logcolor = FALSE, method = c("linear","quantile","jenks")) {
	...
	if (mode(rates) == "numeric") {
		...
		if (tolower(spex) == "s") {
			...
			if (logcolor == FALSE) {
				...
				if (method == 'jenks') {
					bks <- getJenksBreaks(rates[[1]] - rates[[2]], k=(NCOLORS + 1))
				}
				if (method == 'linear') {
				...
				if (method == 'jenks') {
					bks <- getJenksBreaks(z, k=(NCOLORS + 1))					
				}
				if (method == 'linear') {
