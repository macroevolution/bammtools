...
##' quantiles must be supplied (these will define shaded polygons)
##' @param ratetype if 'auto', defaults to speciation (for diversification) or beta
##' (for traits). Can alternatively specify 'netdiv' or 'extinction'. 
...
##' @param smoothParam loess smoothing parameter, ignored if smooth = FALSE.
...
##' \code{avgCol = NULL}).
##' @param start.time start time (in units before present). If NULL, starts at root.
##' @param end.time end time (in units before present). If NULL, ends at present.
##' @param node if supplied, the clade descended from this node will be used or
##' ignored, depending on \code{nodetype}.
##' @param nodetype if 'include', rates will be plotted only for the clade descended
##' from \code{node}. If 'exclude', the clade descended from \code{node} will be
##' left out of the calculation of rates. 
##' @param plot a logical: if TRUE, a plot will be returned, if FALSE, the data
##' for the plot will be returned.
...
##' @param xticks number of ticks on the x-axis, automatically inferred if \code{NULL}.
##' @param yticks number of ticks on the y-axis, automatically inferred if \code{NULL}.
##' @param xlim vector of length 2 with min and max times for x axis. X axis is
##' time since present, so if plotting till the present, \code{xlim[2]==0}. Can
##' also be 'auto'.
##' @param ylim vector of length 2 with min and max rates for y axis. Can also be 'auto'.
##' @param add a logical: should rates be added to an existing plot.
##' @param axis.labels a logical: if TRUE, axis labels will be plotted.
##'
##' @details If the input \code{ephy} object has been generated by \code{\link{getEventData}} and 
...
##' plotting parameters.
##'
##' @return If \code{plot = FALSE}, then a list is returned with the following components:
##'
##' \itemize{
...
##' @author Pascal Title
##'
##' @seealso See \code{\link{getEventData}} and \code{\link{getRateThroughTimeMatrix}} to
##' generate input data.
##'
##' @examples
##' \dontrun{
