...
##'
##' @description If the user would like to specify species sampling on a clade-by-clade
##' basis, a sampling probability table can be provided to BAMM.
##'
##' @param tree an object of class \code{phylo}.
##' @param cladeTable a dataframe with one column of species names and a second column of
##' group assignment. 
##' @param cladeRichness either \code{NULL} or a vector of species counts, named by clade names.
##' @param globalSampling percent sampling for the backbone of the phylogeny
##' @param output path + output file name
##' @param writeToDisk a logical, should the table be written to disk, defaults to \code{TRUE}.
##'
##' @details This function handles two types of input: The cladeTable can either contain
##' the species found in the phylogeny, along with clade assignment of those species, or it
##' can contain more species than found in the phylogeny. If the table only contains those
##' species in the phylogeny, then a vector \code{cladeRichness} must be provided that
##' contains known clade richness. If the cladeTable contains more than the species in the
##' phylogeny, then cladeRichness should be set to \code{NULL}. The \code{globalSampling}
##' value should represent the overall completeness of species sampling in terms of major
##' clades. See \url{http://www.bamm-project.org} for additional details.
##'
##' @return If \code{writeToDisk = TRUE}, then no object is returned. If
##' \code{writeToDisk = FALSE}, then a dataframe is returned. The resultant table must
##' contain one row for each species in the phylogeny, along with clade assignment, and
##' sampling fraction. The first line must contain the overall sampling fraction for the
