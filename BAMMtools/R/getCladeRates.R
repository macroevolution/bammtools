...
##'
##' @details Computes the time-weighted mean evolutionary rate for a given clade. Conversely, one
##' can compute the rate for a given phylogeny while excluding a clade; this operation will give
##' the "background" rate. It is important to understand several aspects of these mean rates.
##' First, rates in the \code{BAMM} framework are not constant through time. Hence, the function
##' computes the mean time-integrated rates across the subtree. Operationally, this is done by
##' integrating the speciation rate with respect to time along each branch in the subtree. These
##' time-integrated rates are then summed, and the sum is divided by the total sum of branch
##' lengths for the subtree. 
##'
##' The function computes a rate for each sample in the posterior, and returns a list of rate
##' vectors. Each rate in the corresponding vector is a mean rate for a particular sample from
##' the posterior. Hence, you can think of the return value for this function as an estimate of
##' the marginal distribution of rates for the focal clade. You can compute means, medians,
##' quantiles, etc from these vectors.
##'
...
##' mean(getCladeRates(ed, node=c(132, 140), nodetype=c('include','exclude'))$lambda)}
##' @export
