...
# nthreads=6
# traitorder='0,1';
traitDependentBAMM <- function(ephy, traits, reps, return.full=FALSE, method='spearman', logrates=T, two.tailed = TRUE, traitorder=NA){

  if (ephy$type != 'diversification'){
    stop("Function currently supports only bammdata object from speciationextinction analyses\n");
  }
  ...
  if (sum(! names(traits) %in% ephy$tip.label)>0){
    ...
    if(length(traits)==0){
      stop("no taxa with trait is in the bammdata object\n")
    }
  }
  ...
  method<-method.option[grep(paste("^",method,sep=''),method.option,ignore.case =T)]
  if(length(method)==0){
...
#check if the traitorder is specified
  ...
  if(two.tailed == F){
    ...
    if(method == 'spearman' | method=="pearson"){
      ...
      direction<-direction.option[grep(paste("^",traitorder,sep=''),direction.option,ignore.case =T)]
      ...
      for (i in trait.state){
        if(sum(traits==i)==0){stop(paste("no species with state ",i," \n",sep=''))}
      }
    }
  }
  ...
  gen<-sample(1:length(tiprates),size = reps,replace = T)

  gen.tiprates<-list()
  for (l in 1:length(gen)){
    gen.tiprates[[l]]<-data.frame(rates=tiprates[[gen[l]]],states=tipstates[[gen[l]]],stringsAsFactors = F);
  }
  ...
  permute_tiprates<-function(m){
      ...
      new_index<-sample(index, size = length(index),replace = F)
      x<-rep(0,length(tt))
      for(xx in 1:length(index)){ x[which(tt==index[xx])]<-lvec[which(index==new_index[xx])]}
      x   
  }  
  if(nthreads>1){
  ...
  cortest<-function(rates,traits,method){    
      return(cor.test(rates, traits, method=method,exact = F)$estimate)   
  }
  manntest<-function(rates, traits, two.tailed,trait.state){
    if( two.tailed ){
      return(wilcox.test(rates~traits,exact = F)$statistic)
    }else {
      return(wilcox.test(rates[which(traits==trait.state[2])],rates[which(traits==trait.state[1])],exact = F)$statistic)
    }
  }
 ...
 if(nthreads>1){
  ...
  if(method=="spearman" |method=="pearson"){
    ...
    l<-lapply(unique(traits[! is.na(traits)]), function(x){median(ave.tiprate[which(traits==x)],na.rm=T)})
