...
##'
##' @description This is one estimate of the "best" rate shift configuration, considering
##' only those shift configurations that were actually sampled using \code{BAMM}'s
##' reversible jump MCMC simulator. This is analogous to the "maximum clade credibility
##' tree" from a Bayesian phylogenetic analysis. It is not necessarily the same as the
##' shift configuration with the maximum a posteriori probability.
##'
...
##'
##' \itemize{
##'     \item{bestconfigs}{ A vector of the index values of MCMC samples with shift
##'     configurations equal to the maximum. Usually, more than one state sampled during
##'     the MCMC simulation will have an identical (maximized) marginal probability. All
##'     samples given in this vector will have an identical shift configuration.}
##'     \item{scores}{The optimality score (product or sum of marginal shift probabilities)
##'     for all sampled shift configurations in the \code{BAMMdata} object}
...
##' @author Dan Rabosky
##'
##' @seealso \code{\link{marginalShiftProbsTree}}, \code{\link{addBAMMshifts}},
##' \code{\link{cumulativeShiftProbsTree}}, \code{\link{credibleShiftSet}},
